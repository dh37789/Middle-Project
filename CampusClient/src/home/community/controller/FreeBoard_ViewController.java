...
public class FreeBoard_ViewController {
	...
	void inReply(ActionEvent event) {
		...
		comvo.setCmt_frbd_id(fvo.getFrbd_id());
		comvo.setCmt_memid(mvo.getMem_id());
		int cnt = 0;
		try {
			cnt = service.insertFbCmt(comvo);
	...
	void modify(ActionEvent event) throws IOException {
		...
		if (ans == ButtonType.OK) {
			pane.getChildren().clear();
			FXMLLoader loader = new FXMLLoader(getClass().getResource("community_freeBoard_up.fxml"));
			Parent root = loader.load();

			FreeBoard_upController upController = loader.getController();
			upController.setMvo(mvo);
			upController.setTable(fvo);
			upController.setFvo(fvo);
	...
	void initialize() throws RemoteException, NotBoundException {
		...
		assert btn_modify != null : "fx:id=\"btn_modify\" was not injected: check your FXML file 'community_freeBoard_view.fxml'.";
		...
		btn_modify.setFocusTraversable(false);
	...
	public void memChk() {
		...
		String fbId = fvo.getFrbd_memId();
		if (memId.equals(fbId)) {
