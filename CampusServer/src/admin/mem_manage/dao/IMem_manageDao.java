 ...
 *
 */
public interface IMem_manageDao{
	/**
	 * 회원 승인이 안되었거나 블랙리스트들을 가져오는 리스트
	 * @return 회원정보를 List해서 가져온다.
	 * @throws RemoteException
	 */
	List<MemberVO> applyList();
	/**
	 * 체크된 사람들의 가입을 승인한다.
	 ...
	 * @throws RemoteException
	 */
	int accessOK(String mem_id);
	/**
	 * 체크된 사람의 가입을 거절한다.
	 * @param mem_id 거절할 회원의 아이디
	 * @return 성공시 1반환
	 */
	int accessNO(String mem_id);
	/**
	 ...
	 * @return
	 */
	int addBlack(String mem_id);
	/**
	 * 회원 아이디를 받아서 일반회원으로 지정한다.
	 * @param mem_id 일반회원으로 지정할 아이디
	 * @return
	 */
	int subBlack(String mem_id);
	/**
	 * 모든 회원 리스트를 가져온다.
	 ...
	 * @return 시청 목록
	 */
	List<VideoJoinVO> getVD(String mem_id);
	/**
	 * 회원의 아이디를 받아서 해당 회원의 문제풀이 현황을 가져온다.
	 * @param mem_id 목록을 확인할 회원의 아이디
	 * @return 풀이 현황
	 */
	List<QuizJoinVO> getQuiz(String mem_id);
	/**
